...
import com.practice.heliguang.livewallpaper.data.IndexBuffer;
import com.practice.heliguang.livewallpaper.data.VertexBuffer;
    ...
    private float[] loadBitmapData(Bitmap bitmap) {
        ...
        for (int row = 0; row < height; row++) {
            for (int col = 0; col < width; col++) {
                ...
                // 生成法线
                // 首先，获取邻接点，然后用这些点创建代表平面的两个向量。
                // 最后，采用两个向量的叉积，并把它归一化得到其表面法线
